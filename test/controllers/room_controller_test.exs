defmodule Peepchat.RoomControllerTest do
  ...
  test "shows chosen resource", %{conn: conn, user: user} do
    ...
    assert json_response(conn, 200)["data"] == %{"id" => to_string(room.id),
      "type" => "room",
      "attributes" => %{
        "name"     => room.name
      },
  ...
  test "updates and renders chosen resource when data is valid", %{conn: conn, user: user} do
    ...
    conn = put conn, room_path(conn, :update, room), data: %{type: "rooms", attributes: @valid_attrs, relationships: %{}}
  ...
  test "does not update chosen resource and renders errors when data is invalid", %{conn: conn, user: user} do
    room = Repo.insert! %Room{owner_id: user.id}
    conn = put conn, room_path(conn, :update, room), data: %{type: "rooms", attributes: @invalid_attrs, relationships: %{}}
    assert json_response(conn, 422)["errors"] != %{}
  end
